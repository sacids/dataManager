...
class Ussd extends CI_Controller
{
    ...
    =================================================*/
    //insert dw data
    function dw_save_data()
    {
        //receive as json
        $post_data = file_get_contents("php://input");
        log_message("debug", 'dawa data  =>' . $post_data);

        $value = json_decode($post_data, TRUE);

        //post data to server
        $data = array(
            'household_no' => $value['dw_kaya'],
            'total_woman' => $value['dw_total_woman'],
            'total_man' => $value['dw_total_man'],
            'total_female' => $value['dw_total_female'],
            'total_male' => $value['dw_total_male'],
            'received_alb_woman' => $value['dw_rec_alb_woma'],
            'received_alb_man' => $value['dw_rec_alb_man'],
            'total_used_alb' => $value['dw_tt_used_alb'],
            'total_dist_alb' => $value['dw_tt_dist_alb'],
            'received_mec_woman' => $value['dw_rec_mec_wom'],
            'received_mec_man' => $value['dw_rec_mec_man'],
            'total_used_mec' => $value['dw_tt_used_mec'],
            'total_dist_mec' => $value['dw_tt_dist_mec'],  
            'mobile' => $value['msisdn'], 
        );

        //insert a data bunch
        $this->model->set_table('vhw_dawa');
        $result = $this->model->insert($data);
