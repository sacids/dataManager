...
class Ohkr_model extends CI_Model
{
    ...
    {
        $query = $this->db->select('symptoms.name as symptom_name, species.name as specie_name,diseases_symptoms.importance')
            ->join('symptoms', 'symptoms.id=diseases_symptoms.symptom_id')
            ->join('species','species.id=diseases_symptoms.specie_id')
            ->get_where('diseases_symptoms', array('diseases_symptoms.disease_id' => $disease_id))->result();
        return $query;
    }
