...
class Perm extends CI_Controller {
	...
	private function set_table_actions() {
		...
		$designs	= $this->Perm_model->get_table_data('ob_design','type','normal');
		...
		$this->db_exp->set_select ( 'template', $templates );
		$this->db_exp->set_button('visibility','design/configure');
	...
	public function manage_table($table_id = false) {
		...
		// initial results
		if($table_id == 3 AND $this->session->userdata('module_id') != 1){
			$cond = "module_id = '".$this->session->userdata('module_id')."'";
		}
	...
	public function table_set_perms() {
		...
		$available_perms = $this->Perm_model->get_all_perms ( $perm_id );
