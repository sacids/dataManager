...
class Xform extends CI_Controller {
	...
	public function submission() {
		...
		$valid_user = $digest_parts ['username']; // TO DO rename valid user to user_name
		                                         
		// get user details from database
		$user = $this->users_model->get_user_details ( $valid_user );
		$valid_pass = $user->digest_password; // digest password TO DO: rename valid_pass to passs
	...
	/*	$query = $this->db->get_where ( 'submission_form', array (
				'user_id' => $user_id,
				'status' => 0 
		) )->row ();
		...
		// call forms
		$this->set_data_file ( FCPATH . "assets/forms/data/xml/" . $user_id );
		$this->load_xml_data ();
		
		// get mysql statement used to insert form data into corresponding table
		$statement = $this->data_insert_query (); // TO DO: change data_insert_query, get_something
		// $this->load->model('xform_model');
		
		echo $this->xform_model->insert_data ( $statement );  // TO DO remove echo, implement logging
	}
	...
	public function initialise($file_name) {
		...
		$statement = $this->create_structure (); // TO DO: change funciton name to get_something
		// $this->load->model('xform_model');
	...
	private function load_xml_data() {
		...
		$this->form_data = array (); // TO DO move to constructor
	...
	private function data_insert_query() {
		...
		// TO DO get_point field assumes there is only one spatial field created
	...
	private function load_xml_definition() {
		...
		echo '<pre>'; print_r($rxml);
		
		// TO DO reference by names instead of integer keys
